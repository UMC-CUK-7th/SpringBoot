...
![~~이름 짓기 더럽게 힘드네~~](Chapter%207%20API%20%E1%84%8B%E1%85%B3%E1%86%BC%E1%84%83%E1%85%A1%E1%86%B8%20%E1%84%90%E1%85%A9%E1%86%BC%E1%84%8B%E1%85%B5%E1%86%AF%20&%20%E1%84%8B%E1%85%A6%E1%84%85%E1%85%A5%20%E1%84%92%E1%85%A2%E1%86%AB%E1%84%83%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A5%20(1)%20a5208c94059f422bb943a56f2481de4d/Untitled.png)
...
- RestContollerAdvice
    ...
    → @RestController 어노테이션이 붙은 컨트롤러에서 발생하는 예외(@ExceptionHandler) 을 AOP 를 적용해 예외를 전역적으로 처리할 수 있는 어노테이션
    
    - 컨트롤러에서 리턴하는 값이 응답값의 body 로 세팅되어 클라이언트에게 전달됨.
    - `RestControllerAdvice` 가 리턴값을 바로 클라이언트에게 전달하는 것과 달리 `ControllerAdvice` 는 리턴값을 기준으로 동일한 이름의 view 를 찾음.
- lombok
    ...
    - 종류
        - @Getter : **컴파일 될 때 getter 메서드들을 생성**
        - @Setter :**컴파일 될 때 setter 메서드들을 생성**
        - @ToString : **toString() 메서드를 생성한다.**
        - @EqualsAndHashCode : **사용 객체에 대해서 equals(), hashCode() 메서드를 생성**
        - @Data : **@Getter(모든속성), @Setter(final이 붙지 않은), @ToString, @EqualsAndHashCode, @RequiredArgsConstructor위의 어노테이션들을 합쳐둔 어노테이션**
        - @NoArgsConstructor: **파라미터(매개변수)가 없는 생성자를 생성**
        - @AllArgsConstructor: **final, @NonNull이 있는 필드를 포함하여 생성자를 생성**
        - @Builder : **해당 클래스에 빌더 패턴을 사용할 수 있도록 해준다.**
        - @Log :  **log라는 변수를 이용하여 로그 기능을 사용할 수 있다**
        - @Log4j ,@Slf4J :  **Log4J(Slf4J) 설정을 이용하여 로그 기능 사용할 수있다. 마찬가지로 log 변수를 통해 사용**
        - @SneakyThrows : **예외 발생시 Throwable 타입으로 반환해준다. JVM(.class) 에서 검사 여부 관계없이 모든 예외에 대해 throw동작 (논란의 여지가 있는 어노테이션 사용시 주의)**
...
> 
> 
> https://github.com/dongjune8931/UMC-work/tree/mission7
> 
